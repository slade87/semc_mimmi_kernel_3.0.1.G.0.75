...
/* USB engineering mode */
static u8 semc_eng;
static int android_set_semc_eng(const char *val, struct kernel_param *kp);
static int android_get_semc_eng(char *buffer, struct kernel_param *kp);
module_param_call(semc_eng, android_set_semc_eng, android_get_semc_eng,
					&semc_eng, 0660);
MODULE_PARM_DESC(semc_eng, "USB engineering mode");
...
static int android_switch_composition(u16 pid)
{
	...
	/* Honour adb users */
	if (dev->adb_enabled) {
		if (semc_eng && func->eng_product_id && func->eng_functions) {
			product_id = func->eng_product_id;
			dev->functions = func->eng_functions;
		} else if (func->adb_product_id && func->adb_functions) {
...
static int android_get_semc_eng(char *buffer, struct kernel_param *kp)
{
	return sprintf(buffer, "%x", semc_eng);
}
...
static int __init android_probe(struct platform_device *pdev)
{
	...
	struct android_dev *dev = _android_dev;
	int ret;
	struct kernel_param *dummy = NULL;
	...
	android_set_sn(pdata->serial_number, dummy);
